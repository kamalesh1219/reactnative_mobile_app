...
const Profile = () => {
  ...
  try {
   ...
   const tamilJson = await tamilRes.json();
    ...
    const combined = [...tamilJson.results, ...englishJson.results].slice(0, 12);
    setMovies(combined);
  } catch (err) {
  ...
  return (
    ...
    <ScrollView className="flex bg-gradient-to-b from-pink-900 via-purple-900 to-black px-4 pt-16  bg-primary">
      {/* Profile Info */}
        ...
        {movies.map((movie, index) => (
          ...
          >
            <Image
              ...
              resizeMode="cover"
            />
            <Text className="text-white text-center text-xs mt-2 mb-1 px-1 truncate">
              {movie.title}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {/* Buttons */}
      <View className="flex-row justify-around mt-6 mb-18">
